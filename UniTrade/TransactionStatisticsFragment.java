...
public class TransactionStatisticsFragment extends Fragment {
    ...
    private List<Transaction> fetchTransactionsForDate(Calendar selectedCalendar) {
        ...
        for (Product product : allProducts) {
            ...
            productCalendar.setTime(product.getListingDate());
            ...
            if (sameDay) {
                // This logic assumes the current user is "u1"
                boolean isBuy = !Objects.equals(product.getSellerId(), "u1");
                transactions.add(new Transaction(
                        product.getName(),
                        product.getFormattedDate(),
                        product.getPrice(),
                        isBuy
                ));
            }
    ...
    private void setupPieChart(List<Transaction> transactions) {
        ...
        pieDataSet.setColors(new int[]{ContextCompat.getColor(requireContext(), R.color.red), ContextCompat.getColor(requireContext(), R.color.green)}); // Red for Buy, Green for Sell
        ...
        buyAmountText.setTextColor(ContextCompat.getColor(requireContext(), R.color.red));
        sellItemsText.setText("Sell : " + sellItems + " items");
        sellAmountText.setText(AppSettings.formatPrice(getContext(), sellAmount));
        sellAmountText.setTextColor(ContextCompat.getColor(requireContext(), R.color.green));
